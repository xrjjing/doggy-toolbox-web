...
├── Naive UI 桌面工具台布局
├── IPC 合约与本机运行时检测
├── Codex / Claude Code SDK bridge 分层入口 + AI 会话历史 + AI 设置持久化
├── 工具工作台 43 个入口（其中 12 个高级面板）
...
├── 统一备份恢复协议（含 nodes / httpCollections / aiSettings）
...
- 统一备份恢复当前覆盖命令、凭证、Prompt、节点、HTTP 集合、AI 设置六块模块数据，并明确按所选模块执行覆盖恢复。
...
- `工具`：当前工作台共 43 个入口，其中 31 个是纯前端算法工具，12 个是高级面板；工具结果可直接交给本机 AI 做复核。
...
- `备份恢复`：导出或覆盖恢复命令、凭证、Prompt、节点、HTTP 集合和 AI 设置数据。
