...
describe('BackupService', () => {
  it('exports commands, credentials, http collections and prompts in one document', async () => {
    const { aiSettingsService, commandService, credentialService, httpCollectionService, backupService } =
    ...
    expect(document).toMatchObject({
      ...
      sections: ['commands', 'credentials', 'prompts', 'httpCollections', 'aiSettings']
    })
  ...
  it('imports selected sections and overwrites existing module data', async () => {
    ...
    const document = await source.backupService.exportBackup({
      sections: ['commands', 'credentials', 'httpCollections']
    })
