...
### Phase M：[x] 截图反馈二次修复：默认收起、详情瘦身、AI 控制分层

目标：继续按最新截图反馈修掉 5 个仍明显影响使用观感的问题。

范围：

- [Image #1] 左侧边栏默认收起，启动后不占据过多横向空间。
- [Image #2] 凭证详情账号/密码展示改成紧凑横向明细，不再出现巨大空白卡片。
- [Image #3] HTTP 页 AI 模型选择移出 HTTP 主发送/保存/环境工具条，作为独立分析控制条。
- [Image #4] AI Chat 会话记录增加删除入口；灵感空间输入框默认值改为空。
- [Image #5] Prompt 模板详情标签改成扁平横向 chips，避免竖向标签和大空白。

交付：

- AppShell 默认折叠侧栏。
- AI Chat 会话删除走主进程 SQLite 历史服务，并带二次确认。
- 凭证、HTTP、Prompt 对应区域样式收紧且保持 Zen-iOS 毛玻璃视觉。

自检记录：

- 发现的问题：侧栏启动默认展开；凭证明细右侧详情被左侧长列表高度撑大；HTTP AI 模型选择混在环境/刷新/导入工具条里；AI Chat 输入框仍有默认提示内容且历史缺删除入口；Prompt 模板标签区仍显得松散。
- 修复结果：AppShell 默认折叠；凭证明细改为内容自适应的紧凑横向账号/密码行；HTTP 新增独立 `AI REVIEW` 毛玻璃控制条；AI Chat 输入初始值改为空，并新增会话删除 IPC / preload / store / UI 二次确认链路；Prompt 标签改为扁平横向 chips 并收紧详情面板间距。
- 二次自检：在唯一 Electron 窗口中逐页点击验证：初始工具页侧栏为收起状态；凭证页详情不再出现大面积空白；HTTP 页 AI 模型选择已移入独立分析条；AI Chat 输入框为空且删除按钮会打开确认弹层，未点击确认删除；Prompt 模板标签横向展示且详情区空白明显减少。
- 剩余风险：会话删除真实确认会删除本地 SQLite 历史，本轮按删除保护规则只验证弹层，不实际确认删除任何会话。
...
- Phase M 开始：按最新 5 张截图反馈继续修复默认侧栏、凭证详情、HTTP AI 控制条、AI Chat 会话删除/空输入框、Prompt 标签横向化。
- Phase M 实机自检完成：只使用当前唯一 Electron 窗口，普通 CSS/renderer 修改通过 reload 检查；未新开第二个 Electron 窗口，未实际删除本地会话数据。
